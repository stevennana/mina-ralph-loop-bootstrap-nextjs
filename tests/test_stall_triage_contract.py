...
class StallTriageContractTests(unittest.TestCase):
    ...
    def test_generated_ralph_docs_and_loop_message_say_triage(self) -> None:
        ...
        self.assertIn("stops the unattended loop for operator triage unless that identical stall has already repeated enough times to auto-branch into RCA", ralph_readme)
        self.assertIn("the loop records the blocker signature first and only auto-branches into the RCA/fix plan", ralph_readme)
